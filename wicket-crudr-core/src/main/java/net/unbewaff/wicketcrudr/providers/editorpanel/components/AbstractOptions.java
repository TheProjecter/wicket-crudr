...
public abstract class AbstractOptions<T> extends FormComponent<T> {
	...
	private final PalettePanel<T> palette;
	 ...
	 *            parent palette
	 */
	public AbstractOptions(final String id, final PalettePanel<T> palette) {
		super(id);
		this.palette = palette;
		setOutputMarkupId(true);
	}

	protected PalettePanel<T> getPalette() {
		return palette;
	}
	...
	public void onComponentTagBody(final MarkupStream markupStream,
		...
		while (options.hasNext()) {
			...
			final CharSequence value;
			{
				...
				String displayString = converter.convertToString(displayValue,
						getLocale());
			...
			if (additionalAttributesMap != null) {
				for (Map.Entry<String, String> entry : additionalAttributesMap
						.entrySet()) {
	...
	protected void avoidAjaxSerialization() {
		getResponse()
				.write(JavaScriptUtils.SCRIPT_OPEN_TAG
