...
public class AjaxEditableLabelContainer<T> extends Panel {
	...
	public AjaxEditableLabelContainer(final String id, final IModel<T> model,
			ILabelProvider<T> labelProvider, IEditorProvider<T> editorProvider, 
			ISurroundingContainerProvider containerProvider) {
		super(id, model);
	...
	protected Component newLabel(MarkupContainer parent, String componentId,IModel<T> model) {
		return labelProvider.newLabel(parent, componentId, model);
	}

	protected FormComponent<T> newEditor(MarkupContainer parent,String componentId, IModel<T> model) {
		return editorProvider.newEditor(parent, componentId, model);
	}
	
	protected WebMarkupContainer newSurroundingPanel(MarkupContainer parent, String componentId) {
		return containerProvider.newSurroundingPanel(parent, componentId);
	}
	...
	private void initLabelAndEditor(final IModel<T> model) {
		...
		container = newSurroundingPanel(this, "editor");
		add(label);
